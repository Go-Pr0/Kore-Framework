...
<vector>
  ...
  <ticket_constraints>
    ...
    <rule>depends_on encodes the execution DAG. Empty array = root wave (starts immediately). Non-empty array = waits for all listed wave numbers. Team-lead runs independent root waves in PARALLEL. Split by DOMAIN, not by file count — the test: could these two waves ship as separate PRs without breaking each other? If yes, both get empty depends_on. If no, the later wave lists the earlier one. depends_on must have no cycles, must reference existing wave numbers only, and at least one root wave must exist.</rule>
